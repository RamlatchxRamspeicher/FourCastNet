...
def ScatterVWrapperV1(input,mp_size):
    ...
    if rank%mp_size == 0:
        ...
        sdispls = [0] + [sum(scounts[:i]) for i in range(1, len(scounts))]
        sdispls = tuple(sdispls)
        sendbuf = comm.as_buffer(input,scounts,sdispls)
        sendbuf = tuple(sendbuf)
    else:
        sendbuf = None
    recvbuf = torch.zeros_like(split_tensors[rank%mp_size],dtype=input.dtype)
    if rank%mp_size == 0:
...
def ScatterVWrapperV2(input,mp_size):
    rank = comm.rank
    split_tensors = split_tensor_along_dim(input, 3, mp_size)
    recvbuf = torch.zeros_like(split_tensors[rank%mp_size])
...
class ScatterGather(torch.autograd.Function):
    ...
    def forward(ctx, input, mp_size):
        ...
        input = ScatterVWrapperV1(input, mp_size)
        return input
...
class GatherScatter(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        if ctx.mp_size == 1:
            return grad_output, None, None, None
        device = ctx.device
        grad_output = ScatterVWrapperV1(grad_output, ctx.mp_size)
